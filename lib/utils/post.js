...
export function getRecommendPost(post, allPosts, count = 6) {
  ...
  // Add null/undefined checks for allPosts
  ...
  for (let i = 0; i < allPosts.length; i++) {
    const p = allPosts[i]
    if (!p || p.id === post.id || !p.type || typeof p.type !== 'string' || !p.type.includes('Post')) {
      continue
    }
...
export function checkSlugHasNoSlash(row) {
  ...
  return (
    ...
    !isHttpLink(slug) &&
    (!row.type || typeof row.type !== 'string' || row.type.indexOf('Menu') < 0)
  )
}
...
export async function processPostData(props, from) {
  ...
  const allPosts = (props.allPages || []).filter(
    page => page && page.type === 'Post' && page.status === 'Published'
  )
  if (allPosts && allPosts.length > 0) {
    const index = allPosts.indexOf(props.post)
    // 只有当找到匹配的post时才设置prev和next
    if (index >= 0) {
