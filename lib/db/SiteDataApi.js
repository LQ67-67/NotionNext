...
if (post?.id && !post?.blockMap) {
  ...
  if (props.post?.blockMap) {
    props.post.blockMap = cleanBlock(props.post.blockMap)
  }
...
function cleanBlock(item) {
  ...
  if (pageBlock) {
    for (const i in pageBlock) {
      ...
      delete pageBlock[i]?.value?.file_ids
      delete pageBlock[i]?.value?.grouped_by
      delete pageBlock[i]?.value?.content_lock
      delete pageBlock[i]?.value?.synced_block
      delete pageBlock[i]?.value?.format?.block_height
      delete pageBlock[i]?.value?.format?.block_width
      delete pageBlock[i]?.value?.format?.block_preserve_whitespace
      delete pageBlock[i]?.value?.format?.block_color
    }
  }
  return post
}
