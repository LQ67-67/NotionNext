...
function cleanBlock(item) {
  ...
  if (pageBlock) {
    for (const i in pageBlock) {
      const block = pageBlock[i]
      if (!block) continue

      // Only clean if it's a block object with value property
      if (block.value) {
        delete block.role
        delete block.value.version
        delete block.value.created_by_table
        delete block.value.created_by_id
        delete block.value.last_edited_by_table
        delete block.value.last_edited_by_id
        delete block.value.space_id
        delete block.value.format?.copied_from_pointer
        delete block.value.format?.block_locked_by
        delete block.value.parent_table
        delete block.value.copied_from_pointer
        delete block.value.copied_from
        delete block.value.permissions
        delete block.value.alive
        // Remove additional fields that are not needed for rendering
        delete block.value.file_ids
        delete block.value.grouped_by
        delete block.value.content_lock
        delete block.value.synced_block
        delete block.value.format?.block_height
        delete block.value.format?.block_width
        delete block.value.format?.block_preserve_whitespace
        delete block.value.format?.block_color
      }
    }
  }
  return post
